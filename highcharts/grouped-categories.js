...
// render ancestor label
    tickProto.addGroupedLabels = function (category) {
        var tick    = this,
            ...
            css     = options.style,
            //keep rotation only for first level
            attr    = { align: 'center'},
...
// set labels position & render categories grid
    tickProto.render = function (index, old, opacity) {
        ...
        while (group = group.parent) {
            var minPos  = tickPosition(tick, mathMax(group.startAt - 1, min - 1));
            var maxPos  = tickPosition(tick, mathMin(group.startAt + group.leaves - 1, max));
